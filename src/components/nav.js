...
const Nav = async () => {
  ...
  //   )}
  // };
  const hideIfPublic = user ? "block" : "hidden";
    ...
    <nav className="flex items-center justify-between bg-teal-500 p-6 dark:bg-gray-800 bg-teal-500 dark:text-gray-400 w-full bottom-0">
      ...
      <div className="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
        <div className="text-sm lg:flex-grow">
          ...
          <Link href='/profile'>
            <button href="#responsive-header" className={`${hideIfPublic} mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4`}>
              Profile
            </button>
          </Link>
        </div>
